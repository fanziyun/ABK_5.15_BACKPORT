...
> **编号让位**：本批起草时叫 Batch 34 / v0.36.0，与并行开着的另一条分支（PR #12，
> 「Batch 34 -- `fuse_fill_write_pages()` 只在无进展重试时预缺页」）撞号 —— 两条都基于
> `110749f`（Batch 33 落地点），对方先占了 34 与 0.36.0 并已开 PR。按本仓先例
> （Batch 33 为三条并行批次让位三次）**改号 34 → 35、版本 0.36.0 → 0.37.0**，锚点
> `#batch-35`。若两条先后合入，`GROUP_COUNTS` 的 core 计数要在后合的那条上合并：本批
> 相对**自己的基点**写 44（基点 core 40 + 本批 4），对方写 41；两者都进去后是 **45**。
