...
| `stable_backport_core` (Batch 40) | `erofs_readahead_relaxed_gfp`: the erofs read path. `z_erofs` decompresses LZ4 **in place**, and when the LZ77 sliding window runs past the pages that happen to be mapped it allocates short-lived bounce pages -- which 5.15 asked for with `GFP_KERNEL \| __GFP_NOFAIL` unconditionally, so a readahead shot could enter direct reclaim for a page it can live without. The group adds `gfp_t gfp` to `struct z_erofs_decompress_req`, sets it per pcluster in `z_erofs_do_read_page()` (`besteffort \|= !fe->readahead` -- note the field is upstream's and reads *inverted* against its own comment, so the tests pin the two flags rather than the name), and reads it in `z_erofs_decompress_pcluster()`. Upstream (`d9281660ff3f`, v6.9) measured -20.2%/-22.6% multi-app-launch on ARM64 Android 5.15 with EROFS 4k pclusters; **no device-side speedup is claimed here.** The device premise was checked per inode rather than from the superblock -- `LZ4_0PADDING` and `lz4_max_distance` carry benign defaults, so `feature_incompat=0x1` alone proves nothing -- and `/vendor/build.prop` really is `EROFS_INODE_FLAT_COMPRESSION`, so the path is live. 5.15 needs none of upstream's new `bool ra` parameter (the frontend already carries `readahead`) and keeps its pcluster struct in `fs/erofs/zdata.h`; two upstream hunks have no 5.15 carrier. The same survey records, with evidence, that **no erofs patch can speed up zram**: the two share only `lib/lz4`, which upstream touches six times between v5.15 and v7.2 with no performance commit among them. |
| `stable_backport_core` (Batch 41) | `vm_kcompressd_swapout`: **Kcompressd-Unofficial 0.5** (Masahito Suzuki, forked from MediaTek's Kcompressd), a per-node `kcompressd%d` kthread that takes the compress-and-swap-out work kswapd performs inside `swap_writepage()` off the reclaim thread, so reclaim latency stops including zram's compression of every anonymous page. This is *not* Batch 10-1's `zram_async_recompress`: that one offloads the second compression of an already-resident page (a used-memory saving), this one offloads the first compression of a page being reclaimed (a reclaim-latency saving). **Upstream ships no 5.15 patch** -- only 6.12.44 / 6.18 / 7.1-rc1 -- because its carrier is four new `struct pglist_data` members and `android/abi_gki_aarch64.xml` pins that struct at 56320 bits across 22 layout-tracked members with no `ANDROID_KABI_RESERVE` run to spend (the four spare slots are in `struct zone`; `ANDROID_OEM_DATA(1)` is one `u64` behind `CONFIG_ANDROID_VENDOR_OEM_DATA` and belongs to the OEM). So this is a re-carriage: the whole payload lands in `mm/page_io.c` -- private `static struct abk_kcompressd_node abk_kcompressd[MAX_NUMNODES]` indexed by `page_to_nid()` instead of pglist_data fields (Batch 10-1's `no struct zram change` pattern), a `late_initcall` instead of `kswapd_run()`/`kswapd_stop()` (which also drops upstream's `kswapd_run()` trap on this tree: it is `void`, has no `__meminit` and no `pgdat_kswapd_lock()`, and AOSP's `trace_android_vh_kswapd_per_node()` can skip the body), and a file-static knob registered with `register_sysctl("vm", ...)` instead of a `kernel/sysctl.c` `vm_table[]` entry. `mm/vmscan.c` is not touched at all. Two upstream gates have no 5.15 symbol and were replaced: `mem_cgroup_zswap_writeback_enabled()` -> this module's own Batch-38 policy `memcg_is_dying(page_memcg(page))`; `zswap_is_enabled()` -> `frontswap_enabled()` (5.15's zswap is a frontswap backend, and `SWP_SYNCHRONOUS_IO` is set on zram). `vm.kcompressd` (default 24) is the FIFO **soft limit**, not its capacity -- the ring is 256 entries; at the limit the oldest page is written out synchronously first, so completion order keeps matching submission order (the improvement over MediaTek's original, which starved the oldest pages and reversed the order). `get_page()` on the queued page is load-bearing: `__remove_mapping()`'s `page_ref_freeze()` wants an exact refcount, so the extra reference is what keeps the swap slot from being handed back while the write is pending -- and it is also why **only kswapd** may offload (direct reclaim would report zero reclaimed on every page and escalate to the OOM killer). The group also fixes a latent double-unlock upstream carries: `shrink_page_list()`'s `keep_locked:` (`mm/vmscan.c:1874`) already unlocks the page before it returns to the LRU, so `do_swapout()` re-takes the lock that `__swap_writepage()`'s every exit will release. **No device-side speedup is claimed.** |
...
The core child carries 66 groups (the 11 pre-Batch-6 grafts plus
...
reaches the LRU -- the module's first `mm/swap.c` group), and Batch 40's erofs
readahead relaxation (`erofs_readahead_relaxed_gfp`: the temporary bounce
pages LZ4/LZMA in-place decompression allocates are requested with
`GFP_NOWAIT|__GFP_NORETRY` on the readahead path and keep
`GFP_KERNEL|__GFP_NOFAIL` for synchronous reads, so a readahead shot no longer
waits in direct reclaim for a page it can live without -- the module's first
`fs/erofs/` group and its first in `fs/` outside `fs/fuse/` and `fs/file.c`),
Batch 41's kcompressd swap-out offload (`vm_kcompressd_swapout`: kswapd's
compress-and-swap-out work moves to a per-node `kcompressd%d` kthread, with a
`memory_notifier` bringing a hot-plugged node's thread up and tearing it back
down again on node removal -- the
module's first `mm/page_io.c` group, and the first group whose per-baseline
shape difference is a *text* probe rather than a config gate, because AOSP's
`android_vh_shrink_page_lock_owner_clear()` vendor hook exists only on the
`android13-5.15-lts` baseline); the
...
`psi_oncpu_state_mask`); the display child carries 1, for 90 groups in total.
