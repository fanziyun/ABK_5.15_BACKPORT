...
The core child carries 41 groups (the 11 pre-Batch-6 grafts plus
...
allocator after `class->lock` is dropped), and Batch 34's FUSE write-path
prefault move (`fuse_prefault_out_of_write_path`: `fuse_fill_write_pages()`
faults its source buffer in only where the copy made no progress, the module's
only group in `fs/fuse/`); the
...
`psi_oncpu_state_mask`); the display child carries 1, for 65 groups in total.
