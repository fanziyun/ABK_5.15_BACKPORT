...
The core child carries 44 groups (the 11 pre-Batch-6 grafts plus
...
allocator after `class->lock` is dropped), and Batch 35's page-cache/page-table
...
whole request's TLB flushes in one `mmu_gather`); the
...
`psi_oncpu_state_mask`); the display child carries 1, for 68 groups in total.
