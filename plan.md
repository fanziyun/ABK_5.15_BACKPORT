...
## 交付总览（九项优化，按功能清单顺序）→ 详见 CHANGELOG.md#overview-nine — 把清单上的九项功能（内存分配 hook / 线程调度 hook / 空实现修复 `offload_all`·`se->slice` / 低内存立刻碎片回收 `abk_gfp_fastfail` / EEVDF / async_depth / zram writeback / bio batching / 重压缩）重排成一份交付日志，并续写第 10 项（per-cgroup PSI 记账开关真正可用：Batch 21→25→26 的可用性链路与 A/B 判定）：逐项给出批次、组名与到手证据（真机 adb、CI 构建号、审计门禁），不新增任何批次
