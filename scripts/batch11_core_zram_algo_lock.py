...
_PARAM_NEW = (
    ...
    "/*\n"
    " * module_param_named, not module_param: the parameter must be called\n"
    " * zram.abk_lock_algo while the variable keeps the abk_zram_ prefix, and\n"
    " * plain module_param() insists on one name for both (it compiles to\n"
    " * \"use of undeclared identifier 'abk_lock_algo'\" otherwise -- caught by\n"
    " * ABK CI, not by the text-level audits).\n"
    " */\n"
...
_LOCK_BODY = (
    ...
    "\tabk_req[abk_n] = '\\0';\n"
    "\n"
    "\tpr_info_ratelimited(\"zram: %s is locked, ignoring a write of '%s'\\n\",\n"
    "\t\t\t    node, abk_req);\n"
    "}\n"
    "\n"
