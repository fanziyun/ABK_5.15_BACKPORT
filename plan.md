...
## companion v0.16.0 + sailboat 附加模块（二）(已落地；单测全绿；**设备未刷入验证**)→ 详见 [CHANGELOG.md](#companion-v0160--sailboat-附加模块二调度侧拆成独立模块) — 主题「把调度相关的(abk_sc_*/abk_sf_*)从主模块完全分离,独立成另一个模块」。**拆的是 companion(用户态)这一侧**,内核 graft 一行未动:KernelSU 模块是用户态、无法携带编译后的内核代码(A/B 两案比较后取「搬迁原模块 1 的东西」)。搬迁是**字节级 move**:`common.sh` 里 449 行 / 10 个函数用 `sed -n '482,930p'` 从 addon 1 逐字节切出,`tunables.conf` 的 sched 块 94 行整段搬走;shared helper(`abk_cfg`/`abk_clamp_uint`/`abk_mul_div`/`abk_pid_*`)是**有意复制**——模块单独刷入,不能读另一个模块的文件。新模块 `ksu/abk_sched_tunables`,id=`abk_sched_tunables`、显示名「sailboat 附加模块（二）」、v0.1.0;addon 1 只改 `name=` 为「sailboat 附加模块（一）」、**id 保持 `abk_runtime_tunables`**(改 id 会让已安装的那份残留成独立模块),版本 v0.15.0 → **v0.16.0**。addon 1 侧删到干净并逐项验证:`common.sh` 1142 → 667 行,`abk_gov_*`/`abk_apply_sched_knobs`/`abk_report_dvfs_state`/`ABK_FAS_NODE`/十个 `sched.abk_*` 键全不在;`service.sh` 去掉 `--supervise-gov`;`action.sh` 去掉 governor 子命令与全部 abk_sf_/abk_sc_/dvfs/fas 行,改为一行指向 addon 2;`embed.conf` 摘掉 fas 工具;README 删段换指针。**打包链路不用改**:`ak3_bundle_ksu_module.py` 的安装循环本就是 `for ... in "$ABK_TU_SRC"/*.zip`,按 glob 装每一个 zip,两个模块一起进同一个 AnyKernel3 zip——这条被新单测钉住,防止将来收窄成单个 id 时 addon 2 静默不再随包下发。**期间自己制造并修掉两个缺陷**:① 在 `ABK_MODULE_SET_ITEMS` 单引号串里写了 `Batch 10-2's` 撇号,提前终止 shell 串(仓库此前特意全去掉过),表现为 12 字段形状断言失败,已去并 `source module.conf` 实测;② heredoc 里 `"\n"` 过度转义在测试文件里留下断串,`py_compile` 抓住。**测试**:新增 `test_sched_tunables_module()` 接管 34 条随搬迁移动的断言,并新增「拆分本身」的断言(addon 1 七个符号必须不在、11 个键两边必须不相交、addon 1 的 action.sh 必须指向这里、AK3 循环必须仍是 glob、显示名与 id 必须分开、cap_pct=90 的实测理由必须同时写在 README 与 tunables),CRLF 断言从硬编码清单升级为扫整个目录;全套 **1329 项全绿**。**设备未刷入验证**:刷入后 addon 2 会与仍在校内的 addon 1 v0.15.0 并存,需一次重启才能完成 v0.16.0(不再写这些节点)与 addon 2 的交接。

## companion v0.15.0(已落地；真机功能已验证；重启后未验)→ 详见 [CHANGELOG.md#companion-v0-15-0](#companion-v0-15-0) — 主题「把 cpufreq governor 全局钉在 schedutil」。**为什么只能在 companion 侧**：真机 `/proc/kallsyms` 的 `__ksymtab_cpufreq_*` 导出表里有 `cpufreq_register_governor`/`unregister_governor`/`get_policy`/`update_policy`/`register_notifier`,**没有任何切换 governor 的入口**(`cpufreq_set_policy` 未导出),而 `drivers/cpufreq/cpufreq.c` 属内建 vmlinux、外部模块塞不进去 ⇒ 唯一通道是 `scaling_governor` 节点。**机制全实测**:该节点出厂 0444 root:root,root 直接写 EACCES;同一 root `chmod 0644` 成功,随后写成功;属主不变故未给非 root 放权,也正是 ROM 自己游戏内切 schedutil 用的同一权限;**全程无 `allow`/`setenforce`/`permissive`**。**关键约束**:chmod 买到的是写权不是持久性 —— vendor 会把 `walt` 写回来(实测一次,约 15 分钟内,且权限位仍停在 0644,即只重写值不重新锁权限;60 秒监听内不回退)⇒ 事件驱动而非持续对抗,写入者未确认(最大嫌疑 `scene-daemon`,它对每 policy 的 `scaling_max_freq` 持 fd 且自行重启),故 `interval=30` 按「一分钟内追上一次变化」定。**耦合**:`abk_apply_early_knobs()` 里 governor 先于 band 生效 —— band 与 `abk_sf_owns()`/`abk_sc_owns()` 都只在 schedutil 下动作,顺序反了就有「band 已武装而 governor 不理它」的窗口;全局 schedutil 同时让 Batch 10-5/42 从「退让」变为「可能接管」(`_rs_foreign` 不再置位),这是预期耦合但未单独测。**交付**:`common.sh` 七个函数(含「先查 `scaling_available_governors` 防拼错旋钮后每轮空写」「写失败才 chmod」「写完回读」「supervisor 连续三轮全拒即自停」「只在行变化时打日志」)/ `tunables.conf` 两个新键(空 = 完全不碰)/ `service.sh` 的 `--supervise-gov`(空键连进程都不起)/ `action.sh governor` 命令 + status 三行。`module.conf` 保持 0.47.0 不动(companion-only 惯例),companion v0.14.0 → **v0.15.0**。**验证**:`stable_5_15_test.py` 新增 22 条断言,顺带把 CRLF 断言从硬编码清单升级为**扫整个目录**(清单外新文件原先可带 CRLF 静默上线),全套 **1292 项全绿**;`bash -n` 四文件过;真机 mksh 实跑四个状态分支 + supervisor 5 轮只留 3 行日志。**未验证**:重启后是否仍成立、写入方身份、性能 A/B(不主张任何数字)。


## Batch 38(v0.43.0,已落地)→ 详见 CHANGELOG.md#batch-38 — 主题「Linux 7.2 MM/Reclaim 筛选后首批落地」，来源 `docs/survey_7_2_mm_reclaim.md`（v7.2 触及 `mm/` 的 commit 共 366 条，用 tag 可达性而非 committer-date 窗口统计）。落地 **6 组**（全 core，core 57 → 63 组）：① `huge_memory_imap_split_uaf`（`e923bd21058e`/5.15.y `f87c08060818`）：`__split_huge_page()` 在释放 after-split 子页**之前**先解 `i_mmap_lock`，堵住并发 `evict()`/`iput()` 把 inode 释放掉之后的 use-after-free；三条 required 步骤，anon 路径 `mapping==NULL` 天然跳过，file 路径调用点随即 `mapping = NULL` 防 `out_unlock` 二次解锁；**lts(216) 已含此补丁 ⇒ PRE_APPLIED**。② `swap_readahead_lru_add_drain`（`a4519e5b648a`）：删掉 swap-in readahead 尾部多余的 `lru_add_drain()`（lru-drain 系列五条里唯一有 5.15 载体的），页面经调用方 per-CPU pagevec 自然入 LRU，省掉每次 swap-in 缺页一次 `lruvec_lock`。③ `vmscan_tasks_rcu_qs`（`25f52e812168`/5.15.y `4cdc1bdf4094`）：`shrink_lruvec()` 扫描循环改报 Tasks-RCU 静止状态；真机核查（vermeer，`5.15.216-...-pr22-4152336`）后**更正**：`/proc/config.gz` 给出 `CONFIG_TASKS_RCU=y` + `CONFIG_TASKS_TRACE_RCU=y`，`call_rcu_tasks`/`synchronize_rcu_tasks` 是真实全局符号，所以 classic holdout 清除确实编进去了——**本组在这台设备上有真实作用**，初版「编译后等价、不主张收益」的说法是错的（方向是少报而非多报）；仍未测速，不主张具体数字。④ `filemap_mmap_miss_tried`（`9b0fcac3cfe7`）：`filemap_map_pages()` 不再把 `FAULT_FLAG_TRIED` 重试计成 mmap hit，修掉 `mmap_miss` 长期低于递增侧导致随机 mmap 预读关不掉的病征（上游随机访问预读 I/O 降约 21×，x86 服务器数据）；与 Batch 30 的 `readahead_mmap_miss_race` 同一条记账线的延续。⑤ `memcg_dying_bailout`（`0beeaf14e7b9`/`e13f634f50d5`/`757dd8193f6c`/`10228e0a5123`）：`memory.high`/`memory.max`/`memory.reclaim`/memcg-v1 四个同步回收写入口在 memcg 已 dying 时提前退出，堵住 `cgroup_rmdir()` 持 `cgroup_mutex` 等在 `kernfs_drain()` 造成的整机卡顿（上游实测 159 s `cgdelete`、182 s 无关读者）；5.15 无 `memcg_is_dying()`，按上游位置补进 `include/linux/memcontrol.h`（CONFIG_MEMCG 块 + `#else` 存根），memcg-v1 两处按 5.15 布局重锚到 `mm/memcontrol.c`；主动回收那条**必须插在 Batch 37 `proactive_reclaim_suspend_abort` 的 `new` 块之后**，插在中间会劈坏该组的幂等探针（trap 5，`step_audit` 第二遍断言抓到）。⑥ `buddyinfo_nolock`（`aaa98b100ea8`）：读 `/proc/buddyinfo` 不再取每个 zone 的 `spin_lock_irqsave`——打印回调只读 `zone->free_area[order].nr_free`，而 lmkd/dumpsys 与厂商碎片监控恰好在分配器最热时轮询该文件；`walk_zones_in_node()` 第四参即 `nolock`，5.15 树上 `pagetypeinfo_showmixedcount()` 已有 `nolock=true` 先例，需要锁的同族回调（`pagetypeinfo_showfree_print`）保持 `false` 不动。**顺带修掉两个既有 fixture/断言缺口**：`tests/fetch_sublevel_tree.sh`+`tests/smoke.sh` 补 `drivers/of/address.c`（overlay 无条件要求该文件存在，新取的树必炸 smoke）、`tests/smoke.sh` 删掉 v0.42.0 移除 `madvise_pt_reclaim`/`madvise_batch_tlb_flush` 后残留的六条陈旧断言。**补上 `tests/implementation_audit.py` 的行为门禁**（recipe §3 要求、本批初版漏了）：六组全部进 `REQUIRED_IN_FUNCTION`，`memcg_dying_bailout` 另在 `REQUIRED_CONTENT` 钉住 `memcg_is_dying()` 的 CONFIG_MEMCG 体与 `#else` 存根；已做反向验证（把组 4 的锚点改回错误的那个 decrement，审计报 missing/forbidden 并退出 1）。**本地 code review 抓到的真问题**：survey 初版把 5.15 的 `mmap_miss` 递减站点写错了行号与函数名（`mm/filemap.c:3119-3125` 属于 `filemap_fault()`，不是 `filemap_map_pages()`），第一版 graft 因此把 guard 加到了错误的 decrement 上；已改到 `filemap_map_pages()` 的 per-PTE 递减并用 `must_not_have` 钉住。**已知局限：只跑了 194 一档**，167/178/216 未取树。**MGLRU 改为默认开启**（本条 PR 的优化全部默认开启）：`LRU_GEN_ENABLED` 从 `_ALIGN_CONFIGS` 挪进 `_MODULE_CONFIGS`，`/sys/kernel/mm/lru_gen/enabled` 开机即非零。理由不是偏好而是正确性：基线本来就 `CONFIG_LRU_GEN=y`，MGLRU 编得进去，但这个符号决定 `mm/vmscan.c` 里 `lru_gen_caps` 是 `DEFINE_STATIC_KEY_ARRAY_TRUE` 还是 `_FALSE`——不设它 MGLRU 每条分支初始为假、跑的是经典 LRU。真机实测（vermeer，本 PR 构建）`LRU_GEN=y` / `LRU_GEN_ENABLED` 未设 / `enabled=0x0000`。**后果：Batch 37 落地的 6 个 MGLRU 组（v6.14 系列）此前一直运行时空转，现在才第一次真正生效**——与 Batch 8 RCU graft 编出去是同一类失败，只是低一层 config。新增 `tests/stable_5_15_test.py::test_mglru_is_enabled_by_the_default_tier` 钉住该放置（反向验证：挪回 align 层 → 6 个检查失败），`_INTRODUCED_KCONFIG` 同步登记。**仍未落地**：v7.2 的 12 条 MGLRU 回收循环重写，所以打开后跑的是 6.1 最小实现 + v6.14 那 6 组，不是 v7.2 那版。其余 opt-in 层（BBR/BLK_WBT/BLK_DEV_THROTTLING/TASK_DELAY_ACCT、per-cgroup PSI、`abk_sf_enable`、伴生 `zram.writeback.trigger`）按用户划定范围**不动**。**未做设备 A/B，不主张提速**。落地脚本 `scripts/batch38_core_mm_safety_perf.py`
## Batch 46(v0.49.0,已落地；六道门禁四档实测全绿；**ABK CI 编译闸门未跑**,**真机未验证**)→ 详见 [CHANGELOG.md](#batch-46) — 主题「**erofs readmore 循环在 EOF 处收口**」。来源 `docs/survey_erofs_upstream.md` §4.1（该 survey 的 §7 建议「先单独落地这一条」）。落地 **1 组**（全 core，core 66 → 67 组）：`erofs_readmore_past_eof`（`936aa701d82d`，mainline v6.5，Chunhai Guo/vivo，2023-07-10，「erofs: avoid useless loops in z_erofs_pcluster_readmore() when reading beyond EOF」）。一字改动：`while (cur >= end)` → `while ((cur >= end) && (cur < i_size_read(inode)))`。**触发场景罕见、不声称提速**：上游复现例是 offset=19217289215、inode_size=1442672，循环 4,691,368 次约 27 秒；commit message 只说「unnecessary should be prevented」，无 CVE、无安全措辞（有 `Fixes:` 但**无 `Cc: stable`**，所以 v6.5 性能修复、任何 5.15 树都不会自带，sublt lts 也不会先到）。**安全论证是查出来的、不是推出来的**：把 `z_erofs_pcluster_readmore()` 在 216 树与 tag v6.5 之间做了整函数 diff——签名（v6.5 后来去掉了 `end`/`pagepool`）、`backmost` 分支（v6.5 改成从 `f->headoffset` 重算 `end`）、循环体控制流（`if (page){}else{}` vs `goto skip`，语义等价）三处差异，没有一处落在循环条件上；所以补上游结论可以直接搬。**锚点因此从 `cur = map->m_la + map->m_llen - 1;` 起**，这三行在两边逐字节相同，而 `backmost` 分支不是。**guard 只会把预读变窄、不会把读出错**：readmore 是预读，调用者真要的那一页由紧随其后的 `z_erofs_do_read_page()` 无条件读；被跳过的页由同步路径自己重新进来读。一个边界细节写明而非抹过：guard 判的是字节偏移 `cur` 而不是页索引，所以落在最后一个不完整页内但 `>= i_size` 的 `cur` 也会被跳过——这是上游原形、不是移植选择，且只有第一次迭代是非页对齐值，代价是少一次预读而非错一次读。**`i_size_read` 可用性查过**：它是 `include/linux/fs.h` 里无条件的 `static inline`（只有 32-bit-SMP 函数体带条件，arm64 无关），而该头文件经 `zdata.c` → `zdata.h` → `internal.h` → `<linux/fs.h>` 可达——这条必须查，因为缺声明正是三道文本审计看不见的 C 级缺陷（AGENTS.md 的 Batch 12 一类）。**顺手补了 Batch 40 留的两个口子**：① `fs/erofs/zdata.c` 是 Batch 40 首次写入的文件（3 步），但 `tests/smoke.sh` 从没断言过它的回滚可还原性，实测还原后逐字节相同，已按仓库惯例补上；② `module.conf` 里 Batch 40 遗留一句 `The request that prompted the batch`（工作日志式归因，Batch 45 只扫了中文措辞所以漏掉），以及一句仍把 zram backport 描述成落在 `5.15.167` 上（Batch 44 撤基线时漏的第 5 处），都已按AGENTS.md 规范改正。`tests/sublevel_matrix.py` 的 `stable_backport_core` 66 → 67，`PRE_APPLIED`/`KNOWN_DEBT` 不变（此提交 `Cc: stable` 无，不可能先到）。

## Batch 45(v0.48.0,已落地；纯文档,零代码改动)→ 详见 [CHANGELOG.md](#batch-45) — 主题「**文档写作规范:面向外部读者,不写工作日志**」。触发点是 `CHANGELOG.md` 里出现「用户要求:xxx」这类归因。**规则已写进 `AGENTS.md` 的 Source-of-truth docs**,适用范围是本仓库发布的每一份文档(`CHANGELOG.md`、`plan.md` 批次索引行、`docs/survey_*`),不是只针对新写内容:不写归因、不用第一人称(我/我们/笔者)、不写「已与用户确认」这类过程叙述;**需求是一件设计事实,就按设计事实写**(「目标:只维护 lts」而非「用户要求只维护 lts」)。**两条豁免也写进了规则**,防后来者误删:「用户」在 userspace/终端用户意义上是技术术语(`fs/file.c` 的 fd table、`kernfs` 的用户态写入者等处大量出现);`自我 DoS` 这类 `自我` 是 self- 的技术前缀。**清扫**:扫全部发布文件类型(`*.md`/`*.py`/`*.sh`/`*.conf`/`*.properties`/`*.rule`/`*.bp`/`*.bzl`/`*.txt`),排除 `build/`、`research/` 等 git-ignored 本地目录;命中 **5 行**(`plan.md` Batch 37/38 索引行 4 处、companion v0.16.0 索引行 1 处、`docs/survey_popsicle_w_611.md` 1 处),全部改掉。其中 companion 那条是唯一删掉会丢信息的:原文「先问过用户 A/B 两条路」记录的是「这个取舍被认真比较过」,改成「A/B 两案比较后取」保留这层信息、只去掉对提问对象的指称。Batch 43/44 的同类措辞在 Batch 44 时已清。**残留 0**(复扫同范围)。`scripts/*.py`/`tests/*`/`module.conf` 一行未动 ⇒ 91 组、`GROUP_COUNTS`、`ABK_MODULE_VERSION` 均不变。

## Batch 44(v0.48.0,已落地；单测与三道树级审计四档全绿,**ABK CI 编译闸门未跑**,**真机未验证**)→ 详见 [CHANGELOG.md](#batch-44) — 主题「**撤掉 5.15.167/.178/.194,只维护 android13-5.15-lts 滚动分支**」。**目标**:撤掉 5.15.167/.178/.194,只维护 android13-5.15-lts 滚动分支。**这不是 registry 变更**：引擎只按文本锚点门控,`ctx.sub_level` 只进报告(`abk_backport_engine.py:38,309,344`),`scripts/` 里数字 sublevel 作为逻辑出现 **0 次**(shape probe 是文本探针,输出 `blocked_by_shape`),`stable_backport.sh` 只有 family 门禁、没有 sublevel 白名单 ⇒ **91 个组一个不改**,`GROUP_COUNTS` 不变。**真正被改的是矩阵与门禁**,因为「支持四档」这句话此前由 `tests/sublevel_matrix.py` 的 `PRE_APPLIED` 四行承载,而 **CI 早就只编 lts**(`abk-kernel-compile.yml` 的 `sub_level:"X"` / `os_patch_level:"lts"`,STATUS_CONTEXT 就是 `ABK kernel compile (android13-5.15-lts)`)——所以本批是让**已验证的事实追上文档的声明**,不是缩小覆盖面。**落地**:`sublevel_matrix.py` 删 167/178/194 三行、`DEFAULT_SUB_LEVEL` 改 "216"(`SUPPORTED` 自动变单元素),`pre_applied()` 的 SystemExit 改写成自解释的滚动重键指引;`stable_5_15_test.py` 把「167 是全 applied 基线」这条跨基线断言**换成单基线等价不变式**——每个前向 graft child 必须仍有组真的改写基线(否则 lts 滚动到把 commit 全收走时,三道树级审计会全部空转却一片绿),另加 `DEFAULT_SUB_LEVEL in SUPPORTED`;硬编码 sublevel 的 fixture 全部改 216(含 autofdo 的 Makefile fixture 与 `_BATCH31_PGTABLE_PRECOMMIT` 改名)。**顺手堵两个洞**:① `smoke.sh` 原先**没有** SUPPORTED 预检,拿一棵不受支持的树来跑会先完整跑一遍 graft、把树全改写、再中途 SystemExit——现在 pass 1 之前就拒;② `step_audit.py:detect_sub_level()` 原先在 Makefile 缺失/畸形时**静默回落到 `DEFAULT_SUB_LEVEL`**,而成员检查又恰好被同一个默认值满足,等于用一棵认不出的树去审 lts 的期望——现在改成 raise。**滚动键的取舍**:维持以 Makefile `SUBLEVEL` 为键(不用稳定 `lts` 标签),因为稳定标签会让陈旧期望静默通过;roll 到 220 时所有审计以 `no expectation recorded for sublevel '220'; supported sublevels: 216` 硬失败,强制重新建键并逐项重新证明。**已写明的真实风险**:CI 编译闸门把 `already_present` 当 GOOD、从不查矩阵,所以 "lts 吸收了某 commit 而该组不在 `PRE_APPLIED`"这一类漂移 **CI 永远绿**,只有本地 step/impl/smoke 能抓到——因此每次重取 lts 后跑本地审计是**强制项**,已写进 `AGENTS.md` 的 "Lts-only maintenance" 与 `docs/porting_policy.md` 同名节。`tools/fetch_all_trees.sh` 也会在取树后自报键与目录不一致。**另修一处与本批无关的文档漂移**:`AGENTS.md` 原写 "`patches/` 和 `files/` 两个目录刻意留空",但 `files/drivers/of/address.c` 真实存在(`files/README.md` 自述是唯一例外,无任何测试钉这条)——已改为准确的 "File payloads (the one exception)" 小节。**未做**:本地 `build/abk-trees/{167,178,194}` 三个旧夹具留在原处(git-ignored,不随仓库发布)。

## Batch 43(政策变更,无新增组)→ 详见 [CHANGELOG.md](#batch-43) — 主题「撤销 `fs/f2fs` / `drivers/scsi/ufs` 两条 sibling 红线，换为注入顺序硬约束」。**目标**：从小米 17 Pro 内核分支 `popsicle-w-oss` 找可移植的 f2fs 优化以改善 /data 读取，并解除 `fs/f2fs` / `drivers/scsi/ufs` 的写入限制。**实测结论——该分支无可用载体**：`popsicle-w-oss` 全树递归 **2686 条目**（GitHub API `truncated:False`，非截断）、**141 个 `modules.bzl`**，是 kleaf/bazel 的 out-of-tree 模块片段树，不含 GKI 主体：`fs/` **0 条目**、`block/` 0、`crypto/` 0、`mm/` 仅 `modules.bzl` + `zsmalloc.c`、`drivers/scsi/` 仅 `modules.bzl` + `sg.c`；f2fs 关键词全树命中 **0**。存储侧仅存的三份文件是 `mm/zsmalloc.c`（zram，Batch 4/6 已覆盖）与 `drivers/ufs/host/{ufs-qcom.c,ufs-qcom.h,ufshcd-crypto-qti.c}`（高通 SoC 专用，且无 `ufshcd.c` 核心，AOSP GKI 无锚点）。267 个分支里也没有 f2fs/common 源分支 —— GKI 公共内核的 `fs/`、`block/`、`crypto/` 由 AOSP 侧提供，MiCode 仓库只放高通胶水层。**因此从该分支一个 f2fs 候选也取不到**；红线撤销并非「为了搬小米的东西」—— 该分支没有可搬载体 —— 而是路径本身按目标放开。f2fs 候选的正源是上游 android15-6.6 / 5.15.y stable（见 `docs/batch8_long_term.md` C 节，其中 `f2fs_lookup_mode_perf` 投入最小、最值得先做）。**撤销后换成什么**：`ABK_F2FS_FIX_MODULE` 的四条 rollback 子模块仍须跑在本模块之前（或干脆不同注该 suite）—— 它的回滚是 `git apply --reverse --check`、patch context 钉在 `android13-5.15-2024-11_r14`，本模块先改写 f2fs/ufs/`block/` 的 F2FS 区域它就会反打失败；原先「保持 footprint 不相交」因此改成「必须能检测到重叠」。**改动范围**：纯文档 —— `AGENTS.md`（Red lines 的 Do-not-touch → Storage lanes are open，Composition order 补顺序约束）、`docs/porting_policy.md`（三模块顺序 item 1 + Footprint disjointness → Footprint overlap）、`docs/survey_suite_absorption.md` §5、`docs/batch8_long_term.md` C 节与执行顺序第 4 条、`plan.md` 禁区清单、`docs/survey_popsicle_w_611.md`（补本次复核实测）、`scripts/stable_backport.sh` 抬头注释。`scripts/*.py` 与 `tests/*` 一行未动 ⇒ `GROUP_COUNTS` 不变、无新 PatchGroup、无新 Kconfig，`ABK_MODULE_VERSION` 保持 **0.47.0** 不递增。

→ 详见 [CHANGELOG.md#batch-42](#batch-42) — 主题「把 WALT `smart_freq` 四件套里唯一没落地的那件补上：`freq_cap[]` → `freq = min(freq, cap)` 上限钳制」。落地 **1 组**（`scripts/batch42_perf_schedutil_smart_cap.py`，perf 23 → 24，模块 `0.46.0` → `0.47.0`），组名 `schedutil_smart_cap`，落点 `kernel/sched/cpufreq_schedutil.c`，hook `android_vh_cpufreq_resolve_freq`（`__resolve_freq()` 里，policy min/max clamp 之后、查频表之前，fast/slow 两路都覆盖，`walt_pelt_survey.md` §3B 认定的最优落点）。**编号说明**：需求原话写「Batch 40, v0.44.0 → 0.45.0」，但仓库当时已落 Batch 40 = erofs（v0.45.0）与 Batch 41 = kcompressd（v0.46.0），复用 `Batch 40` 会让一个编号指向两组，故取 42 / v0.47.0；组名、落点、旋钮、语义、闸门与需求逐条一致。**补的是哪一半**：`CHANGELOG.md#batch-10-2` 的范围是「reason 选举 + 去激活迟回 + freq 上限钳制 + per-cluster 阈值表」四件，`schedutil_smart_policy` 落了前两件，**上限钳制从设计草案起就写着、从未落地**；§4.1 四项可保留物 (a) 已落、(b) 即本批、(c) 不落（5.15 cpufreq 无 `walt_sched_cluster`，按 policy 取状态是等价且更小的形态）。**上游行号**：`smart_freq.c:433` `smart_freq_update_one_cluster()`（cap 选举）、`smart_freq.c:505` `UNCAP_THRES 300000000` / `UTIL_THRESHOLD 90`（= `abk_sc_hold_ms` / `abk_sc_entry_pct`）、`cpufreq_walt.c:264` `get_smart_freq_limit()` 的 `if (freq > smart_freq) freq = smart_freq`（= `*target_freq = cap;`）。**死区教训（硬性）**：`schedutil_smart_policy` 第一种形态按 util 时间窗退出（≥90% 进入、≥70% 续期、连续 250ms <70% 才退出），实机上造成 **70%–90% 死区** —— util 稳在 75–85% 时退出窗口永不成立，reason 永不清除、频率被摁住不降（用户实测复现过）。故本批放开判据**只用频率请求方向**（最近一次被压制的请求仍高于 cap 且连续持续 `abk_sc_hold_ms`），util 窗口只用于**相反一侧**（util 跌破 `abk_sc_release_pct` 保持 `abk_sc_release_ms` → 收回已放开的 cap）—— 一个只能让功能重新上线的窗口不可能把频率摁住不放，旧故障形态在这里没有可咬的东西；`abk_sc_entry_pct` 只喂只读 reason 位图、不 gate 任何决策。payload 里以 `ABK_SC_RELEASE IS NOT A UTIL WINDOW` 注释块逐字引用了该教训。**耦合与定序**：两者同一 hook 同一文件，`android_vh` 探针按注册顺序执行 ⇒ 规定「cap 先于 floor」；两者都是 `late_initcall` 且在同一个 TU，所以就是文本顺序 —— cap payload 嫁接在 `cpufreq_governor_init(schedutil_gov);` **这一行前面**，这是同时满足「文本先于 floor」「与引擎先跑哪个组无关（两序收敛，单测双向断言）」「对两组都幂等」的唯一位置（接在锚点之后会把 payload 夹在锚点与 floor 之间，第二遍 floor 的 `new` 不匹配而 `old` 仍匹配 ⇒ 再 append 一份，即 group_recipe §2 trap 5 / Batch 21 `psi_account_irqtime()` 事故）。「cap 先跑」还不够（floor 后跑且只会抬），故 cap 在 `late_initcall_sync`（7s，排在所有 7 级条目之后含 floor 的 `late_initcall`）再注册一个尾探针，只在本趟已压过且有后来的探针抬回 cap 之上时重压 —— **cap 生效时 floor 让位**；之所以不靠改 floor payload：它是 Batch 10-5 的字节冻结迁移锚点，改它会让已嫁接的树同时不匹配 `has_current_policy`/`has_legacy_policy` ⇒ 整组 `blocked_by_shape` 堵死每棵已嫁接的树。**状态**：per-policy + `raw_spinlock_t`（`__resolve_freq()` 在 `fast_switch` 路径、`rq->lock` 下被同 policy 多 CPU 并发调用），**没有抄社区 schedhorizon fork 那个无锁 `current_step`**（类型不匹配、表长度自相矛盾、调试打印残留）。**闸门四条全在** `abk_sc_owns()`：默认 `false`、`policy->governor` 名非 `"schedutil"` 即 return、`policy->min == policy->max` 拒绝、`cap <= policy->min` 或 `cap >= policy->max` 直接 return（后两者任何一个应用了都是 Batch 10-4c 锁频，从这一侧看）。旋钮 6 个 0644（`abk_sc_enable`/`abk_sc_cap_pct`=100/`abk_sc_hold_ms`=300/`abk_sc_entry_pct`=90/`abk_sc_release_pct`=70/`abk_sc_release_ms`=250）+ 只读 0444 两个（`abk_sc_boosting` CPU 位图、`abk_sc_capped` policy 位图按 `policy->cpu` 编号），全 `abk_sc_` 前缀与 `abk_sf_` 零重名；`abk_sc_dvfs_owned()` 是 ownership gate 的本 payload 私有副本（同名同 TU 会编译冲突）。**验证**：`stable_5_15_test.py`（本批 63 项新断言，含 cap/floor 双向顺序、payload 不含 util 窗口 idiom、未知形状拒写）/ `step_audit.py` / `implementation_audit.py` / `smoke.sh` 在 **167/178/194/216 四档全绿**，`GROUP_COUNTS["stable_perf_backport"]` 23 → 24，四档 dry-run `schedutil_smart_cap: applied`、无 `blocked_by_shape`；`py_compile` + `bash -n` 过。`abi_symbollist` 论证不变（嫁接文件非 static 顶层声明数、`EXPORT_SYMBOL`=0、`ANDROID_KABI`=0、`#if` 数均与嫁接前一致；不新增 Kconfig）。**`config_gate_audit.py` 在补丁树上失败一处、与本批无关**：`mm/page_alloc.c:1403` 的 `CONFIG_HIGHMEM` 在 `research/config_audit/vermeer-5.15.216-ci.config` 里 absent；已用 `git archive HEAD` 的 pristine 模块在同一棵树上复现同一失败，确认是 `AGENTS.md` 明说的旧 `.config`（v0.23.0/Batch 18 真机构建产物）tier/config 矛盾。**ABK CI 编译闸门未跑**（外部工作流，本机无 API key/构建），**运行时未验证**：当前真机 vermeer 刷 `5.15.216-android13-8-g5bfe2b8c1439`，该构建 `abk_sf_*` 符号 0 个、`/sys/module/cpufreq_schedutil/parameters/` 整个目录不存在 —— 机上没有本模块任何 payload，**不能声称真机验证过**；须刷带 `stable_perf_backport` 的 build 后复测（两个只读节点出现与否、默认行为与嫁接前逐场景对比、§3 死区是否真的不再出现）。
...
- [-] `1119609dce0875`（ACK：「EEVDF scheduling fail → 取 leftmost」）：**已覆盖**。选择器的 `if (!best)` leftmost 回退早在 `scripts/batch15_perf_eevdf.py:1138`（效果等价于它的 hunk 1），hunk 2 依赖 5.15 没有的 `se->sched_delayed`，而它那条 `printk_deferred` 照抄会误报（本模块的 skip 判定在扫描**内**）；本次只补上缺的钉子。逐调用点核对与理由见 `docs/survey_eevdf_gap.md` §7.1
...
- ~~不在本模块内回滚/前向改写 `fs/f2fs`、`drivers/scsi/ufs`（F2FS 套件领地）~~
  **Batch 43 撤销**：两条均开放为合法 graft 目标。撤销的不是一条边界，而是把它
  换成了一条**硬顺序约束** —— 本模块必须排在 `ABK_F2FS_FIX_MODULE` 的 rollback
  之后注入（或干脆不同注该 suite），因为该 suite 的回滚是
  `git apply --reverse --check`（`android13-5.15-2024-11_r14` 的 patch context），
  本模块先改 f2fs/ufs/`block/` 的 F2FS 区域，它就反打不过。具体要求见
  `docs/porting_policy.md` "Footprint overlap"；截至 Batch 43 尚无组落在这两条
  路径上，因此这是已记录的约束而非当前故障。
